...
class BigGAN_Res_Deep(nn.Module):
    # Inputs:
    ...
    def __init__(self, inCh, outCh, useCls=False, num_cls=0, cls_dim=128):
        super(BigGAN_Res_Deep, self).__init__()
        ...
        self.Conv2 = nn.Conv2d(inCh//4, inCh//4, 3, padding=1)  # (N, inCh/4, L, W)
        
        if useCls:
            self.clsProj3 = nn.Linear(cls_dim, num_cls)
            self.BN3 = ConditionalBatchNorm2d(inCh//4, num_cls) # (N, inCh/4, L, W)
        else:
            self.BN3 = nn.BatchNorm2d(inCh//4)                  # (N, inCh/4, L, W)
        self.Act3 = nn.ReLU()                                   # (N, inCh/4, L, W)
        self.Conv3 = nn.Conv2d(inCh//4, inCh//4, 3, padding=1)  # (N, inCh/4, L, W)
        
        if useCls:
            self.clsProj4 = nn.Linear(cls_dim, num_cls)
            self.BN4 = ConditionalBatchNorm2d(inCh//4, num_cls) # (N, inCh/4, L, W)
        else:
            self.BN4 = nn.BatchNorm2d(inCh//4)                  # (N, inCh/4, L, W)
        self.Act4 = nn.ReLU()                                   # (N, inCh/4, L, W)
        self.Conv4 = nn.Conv2d(inCh//4, outCh, 1)               # (N, outCh, L, W)
